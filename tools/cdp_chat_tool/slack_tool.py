...
class SlackTool:
    ...
    def search_by_jira_key(self, jira_key: str, channels: List[str] = None, limit: int = 10) -> List[Dict]:
        """Search Slack messages that contain references to a specific JIRA key"""
        try:
            logger.info(f"Searching Slack for JIRA key: {jira_key}")
            
            # Use channels from environment if not provided
            if channels is None:
                env_channels = os.getenv('SLACK_SEARCH_CHANNELS', '').split(',')
                channels = [ch.strip() for ch in env_channels if ch.strip()]
            
            # Build search query for the JIRA key
            query = jira_key
            ...
            if channels:
                ...
                all_results = []
                for channel in channels:
                    channel_query = f"{jira_key} in:#{channel}"
                    channel_results = self._search_messages_by_query(channel_query, limit)
                    if channel_results:
                        for result in channel_results:
                            result['search_channel'] = channel
                        all_results.extend(channel_results)
                return self._dedupe_slack_results(all_results)[:limit]
            else:
                # Search across all accessible channels
                results = self._search_messages_by_query(query, limit)
                return self._dedupe_slack_results(results)
                
        except Exception as e:
            logger.error(f"Error searching by JIRA key {jira_key}: {e}")
            return []

    def _search_messages_by_query(self, query: str, limit: int = 10) -> List[Dict]:
        """Helper method to search messages by query using available API"""
        results = []
        
        if self.user_client:
            try:
                response = self.user_client.search_messages(query=query, count=limit)
                if response and response.get('ok') and 'messages' in response:
                    messages = response.get('messages', {})
                    matches = messages.get('matches', []) if messages else []
                    ...
                    for match in matches:
                        ...
                        result = {
                            ...
                            'thread_summary': '',
                            'reply_count': 0,
                            'jira_tickets': self._extract_jira_tickets_from_text(match.get('text', ''))
                        }
                        
                        # Try to get thread information
                        if result['channel'] and result['ts']:
                            thread_summary = self._get_thread_summary_from_search(
                                result['channel'], 
                                result['ts'], 
                                query
                            )
                ...
                logger.warning(f"Search API failed, falling back to conversation history: {e}")
                # Fallback to searching conversation history
                fallback_results = self._search_via_conversation_history(query, [], limit)
                return fallback_results if fallback_results else []
        else:
            # Use conversation history fallback
            fallback_results = self._search_via_conversation_history(query, [], limit)
            return fallback_results if fallback_results else []
        
        return results

    def _extract_jira_tickets_from_text(self, text: str) -> List[str]:
        """Extract JIRA ticket keys from text using the same patterns as thread analysis"""
        if not text:
            return []
        ...
        for pattern in jira_patterns:
            matches = re.findall(pattern, text, re.IGNORECASE)
            for match in matches:
                ticket_key = match if isinstance(match, str) else (match[0] if match and len(match) > 0 else None)
                if ticket_key and ticket_key not in jira_tickets:
                    jira_tickets.append(ticket_key.upper())
        
        return jira_tickets

    def _dedupe_slack_results(self, results: List[Dict]) -> List[Dict]:
        """Deduplicate Slack thread results based on timestamp and channel"""
        if not results:
            return []
        ...
        for result in results:
            ...
            # Create a unique identifier for each thread/message
            channel = result.get('channel', '')
            ts = result.get('ts', '')
            thread_ts = result.get('thread_ts', ts)  # Use thread_ts if available, otherwise use ts
            
            # For thread deduplication, use the thread timestamp
            thread_id = f"{channel}:{thread_ts}"
            
            if thread_id not in seen_threads:
                seen_threads.add(thread_id)
                unique_results.append(result)
                logger.debug(f"Added unique thread: {thread_id}")
            else:
        ...
        logger.info(f"Deduplication: {len(results)} -> {len(unique_results)} unique threads")
        return unique_results

    def search_enhanced_with_jira_dedup(self, query: str, channels: List[str] = None, limit: int = 10) -> Dict:
        """Enhanced search that combines regular search with JIRA key extraction and deduplication"""
        results = {
            'slack_messages': [],
            'jira_keys_found': [],
            'deduplicated_threads': [],
            'total_messages_before_dedup': 0
        }
        ...
        try:
            ...
            if channels:
                slack_results = self.search_in_channels(query, channels, limit)
            else:
                slack_results = self.search_messages(query, limit)
            
            if not slack_results:
                slack_results = []
                
            results['total_messages_before_dedup'] = len(slack_results)
            
            # Step 2: Extract JIRA keys from all messages
            all_jira_keys = set()
            for msg in slack_results:
                if not msg:
                    continue
                    
                jira_keys = self._extract_jira_tickets_from_text(msg.get('text', ''))
                all_jira_keys.update(jira_keys)
                msg['jira_tickets'] = jira_keys
                
                # Also extract from thread summary if available
                thread_summary = msg.get('thread_summary', '')
                if thread_summary:
                    thread_jira_keys = self._extract_jira_tickets_from_text(thread_summary)
                    all_jira_keys.update(thread_jira_keys)
                    msg['jira_tickets'].extend(thread_jira_keys)
                    msg['jira_tickets'] = list(set(msg['jira_tickets']))  # Remove duplicates
            
            results['jira_keys_found'] = list(all_jira_keys)
            
            # Step 3: For each JIRA key found, search for additional Slack threads
            additional_results = []
            for jira_key in all_jira_keys:
                logger.info(f"Searching additional threads for JIRA key: {jira_key}")
                jira_results = self.search_by_jira_key(jira_key, channels, limit=5)
                if jira_results:
                    additional_results.extend(jira_results)
            
            # Step 4: Combine all results and deduplicate
            all_results = slack_results + additional_results
            deduplicated_results = self._dedupe_slack_results(all_results)
            
            results['slack_messages'] = deduplicated_results[:limit]
            results['deduplicated_threads'] = deduplicated_results
            
            logger.info(f"Enhanced search complete: {results['total_messages_before_dedup']} -> {len(deduplicated_results)} after dedup, {len(all_jira_keys)} JIRA keys found")
            
        except Exception as e:
            logger.error(f"Error in enhanced search with JIRA dedup: {e}")
